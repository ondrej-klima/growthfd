...
for(colName in colNames) {
  ...
  growthfd.bgs.plotIndividuals(age, ids, apvs, values, vel, acc, gather, sprintf('%s_9.pdf', colName))
  growthfd.bgs.plotIndividualsPoints(unique(res[[2]]), gather,  sprintf('%s_pts_9.pdf', colName))
}
...
growthfd.plotwarps(model, age=seq(min(9, 18, 0.05)))
